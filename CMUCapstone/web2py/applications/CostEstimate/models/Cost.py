class Cost(object):
    ...
    def __init__(self, tsp_solution):
        self.tsp_solution = tsp_solution
        self.cluster_solution = None
    ...
    def cost_calculation(self):
        ...
        #testval=table[0][2]
        #costPerCaseTable=[[ for x in xrange(len(TSPtable))] for x in xrange(3)]
        costPerCaseTable=[]
        #costPerCaseTable.append([])
        for rowno in range(len(TSPtable)):
            #print("**************Row No ********"+str(rowno))
            #costPerCaseTable.append([])
            #print("testvalue is"+str(testval))
            #nodeNum=input('Enter the Node Number to find cluster and cost:')
            #nodeNum=row[3]
            nodeNum=TSPtable[rowno][3]
            #print(nodeNum)
            intNodeNum=int(nodeNum)  # node number to find cost
            #clusterNo=0
            if(intNodeNum!=0):
                costPerCaseTable.append([])
            for row in table:
                if(row[0]==intNodeNum):
                    #print("Inside if")
                    clusterNo=row[1] # cluster to which node belongs to
        clusterNodeList=[];
        #print(clusterNo)
        ...
        if(intNodeNum!=0):
            costPerCaseTable[rowno].append(intNodeNum)
            costPerCaseTable[rowno].append(nodeCostPerCase)
            costPerCaseTable[rowno].append(clusterNo)
        print costPerCaseTable

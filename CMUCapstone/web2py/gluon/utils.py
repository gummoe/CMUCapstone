    ...
    if address.lower() in ('127.0.0.1', 'localhost', '::1', '::ffff:127.0.0.1'):
        ...
        if hasattr(socket, 'inet_aton'):  # try validate using the OS
            try:
                ...
                return True
            except socket.error:  # invalid addresses
                return False
        ...
        try:
            ...
            return False
    else:  # do not know what to do? assume it is a valid addresses
        return True


def is_loopback_ip_address(ip=None, addrinfo=None):
    """
    Determines whether the addresses appears to be a loopback addresses.
